...
# dvc exp run --queue -S "model.lora_rank=32" -S "model.lora_alpha=64" -n "esm2_lora_base_rank4_alpha32"
...
dvc exp run --queue -S "datamodule=base_allseq_exc_cluster" -S "model.learning_rate=5e-4" -n "esm2_lora_base_allseq_exc_cluster_lr5e-4"
dvc queue status